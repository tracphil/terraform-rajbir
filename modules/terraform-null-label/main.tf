...
locals {
  defaults = {
    label_order = ["namespace", "account", "stage", "name", "location", "attributes"]
    delimiter   = "-"
    replacement = ""
  ...
  location           = lower(replace(coalesce(var.location, var.context.location, local.defaults.sentinel), local.regex_replace_chars, local.defaults.replacement))
  ...
  account            = lower(replace(coalesce(var.account, var.context.account, local.defaults.sentinel), local.regex_replace_chars, local.defaults.replacement))
